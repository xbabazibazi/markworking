...
# Qt'nin C++ seviyesindeki tüm ekran/DPI loglarını işletim sistemi düzeyinde kapatır
...
class JarvisLive:
    ...
    async def _execute_tool(self, fc) -> types.FunctionResponse:
        ...
        try:
            if name == "open_app":
                ...
                # r liste ise metne çevir, değilse doğrudan string olarak al
                r_text = "\n".join(map(str, r)) if isinstance(r, list) else str(r)

                if r_text and not r_text.startswith("No results") and not r_text.startswith("Search failed"):
                    _query = args.get("query") or ", ".join(args.get("items", []))
                    _label = f"{_mode.upper()} — {_query[:38]}" if _query else _mode.upper()
                    self.ui.show_content(_label, r_text)
    ...
    async def _save_session_summary(self) -> None:
        ...
        try:
            ...
            # resp.text'e doğrudan dokunmadan önce 'thought' parçalarını süzüyoruz
            # Bu sayede terminaldeki 'non-data parts' uyarısı tamamen engellenir
            summary_parts = []
            ...
            # Eğer süzme boş dönerse yedek olarak güvenli alıyoruz
            if not summary and hasattr(resp, "text") and resp.text:
