...
class HudCanvas(QWidget):
    def __init__(self, face_path: str, assistant_name: str = "ZYRON", parent=None):
...
class LogWidget(QTextEdit):
    ...
    def __init__(self, parent=None):
        ...
        self._pos     = 0
        self._tag     = "sys"
        self._ai_name_lc = "zyron"   # updated when assistant name changes
        self._tmr = QTimer(self)
        self._tmr.timeout.connect(self._step)
    ...
    def _next(self):
        ...
        elif tl.startswith(_ai_pfx) or tl.startswith("zyron:"): self._tag = "ai"
...
class FileDropZone(QWidget):
    ...
    def _browse(self):
        path, _ = QFileDialog.getOpenFileName(
            self, "Select a file for ZYRON", str(Path.home()),
            "All Files (*.*);;"
...
class SetupOverlay(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        layout.addWidget(_lbl("Configure ZYRON. before first boot.", 9, color=C.PRI_DIM))
        layout.addSpacing(6)
...
class CustomizeOverlay(QWidget):
    ...
    def __init__(self, assistant_name="ZYRON", user_name="",
    ...
    def _save(self):
        name = self._name_input.text().strip() or "ZYRON"
...
class ClipboardPanel(QWidget):
    """Floating panel shown when text is copied — offers quick Zyron actions."""
...
class RemoteKeyOverlay(QWidget):
    ...
    def mark_connected(self) -> None:
        ...
        self._timer_lbl.setText("Phone connected — ZYRON ready")
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, face_path: str):
        ...
        self._assistant_name: str = (_cfg.get("assistant_name") or "ZYRON").strip()
        ...
        self.on_interrupt      = None   # callable: () -> None — stop ZYRON mid-speech
        self.get_plugins       = None   # callable: () -> list[dict], set by ZyronLive
        self._muted            = False
        self._current_file: str | None = None
    ...
    def _setup_tray(self):
        ico_path = Path(__file__).resolve().parent / "config" / "zyron.ico"
    ...
    # ------------------------------------------------------------------
    @staticmethod
    def _build_zyron_icon(out_path: Path) -> bool:
        """
        Render a ZYRON arc-reactor icon at 4× resolution and downsample
    ...
    def _create_lnk_windows(lnk: str, target: str, args: str,
        ...
        try:
            ...
            sc.Description      = "ZYRON AI Assistant"
    ...
    def _create_desktop_shortcut(self):
        ...
        if not ico_path.exists():
            self._build_zyron_icon(ico_path)
        ...
        try:
            ...
            # ── macOS — proper .app bundle (no Terminal window) ───────────────
                ...
                (app / "Contents" / "Info.plist").write_text(
                    ...
                    '  <key>CFBundleExecutable</key><string>ZYRON</string>\n'
                    '  <key>CFBundleIdentifier</key>'
                    '<string>com.zyron.assistant</string>\n'
                    '  <key>CFBundleName</key><string>ZYRON</string>\n'
    ...
    def _check_autostart(self) -> bool:
        ...
        try:
            if _OS == "Windows":
                ...
                try:
                    winreg.QueryValueEx(key, "ZYRON_AI")
                    return True
                ...
                return (Path.home() / "Library" / "LaunchAgents"
                        / "com.zyron.assistant.plist").exists()
            else:
                return (Path.home() / ".config" / "autostart" / "zyron.desktop").exists()
        except Exception:
            return False
    ...
    def _toggle_autostart(self):
        ...
        try:
            ...
            if _OS == "Windows":
                ...
                if currently_on:
                    winreg.DeleteValue(reg, "ZYRON_AI")
                else:
                    pythonw = Path(sys.executable).parent / "pythonw.exe"
                    exe = str(pythonw if pythonw.exists() else sys.executable)
                    winreg.SetValueEx(reg, "ZYRON_AI", 0, winreg.REG_SZ,
                                      f'"{exe}" "{script}"')
                winreg.CloseKey(reg)
            elif _OS == "Darwin":
                plist_dir = Path.home() / "Library" / "LaunchAgents"
                plist_dir.mkdir(parents=True, exist_ok=True)
                plist = plist_dir / "com.zyron.assistant.plist"
                if currently_on:
                    ...
                    plist.write_text(
                        ...
                        '"http://www.apple.com/DTDs/PropertyList-1.0.dtd">\n'
                        '<plist version="1.0"><dict>\n'
                        '  <key>Label</key><string>com.zyron.assistant</string>\n'
    ...
    def _open_customize(self):
        ...
        cw = self.centralWidget()
        ov = CustomizeOverlay(
            cfg.get("assistant_name", "ZYRON") or "ZYRON",
            cfg.get("user_name", ""),
    ...
    def _apply_name_update(self, name: str, user_name: str, ui_color: str = ""):
        ...
        self._assistant_name = name.strip() or "ZYRON"
        display = self._assistant_name.upper()
        self.setWindowTitle(f"{display} — MARK LI")
        self._title_lbl.setText(display)
        if display in ("ZYRON", "ZYRON"):
    ...
    def _on_setup_done(self, key: str, os_name: str):
        ...
        if self._overlay:
            ...
            self._overlay = None
        self._apply_state("LISTENING")
        self._assistant_name = _read_full_config().get("assistant_name", "ZYRON") or "ZYRON"
