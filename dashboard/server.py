"""
dashboard/server.py — ZYRON Local HTTP Dashboard
...
def _make_uploads_dir() -> Path:
    ...
    for candidate in [
        Path.home() / "Downloads" / "ZYRON Uploads",
        Path.home() / "Documents" / "ZYRON Uploads",
        BASE_DIR / "uploads",
    ]:
        try:
...
# ── AES-256-CBC ───────────────────────────────────────────────────────────────
_AES_SALT = b'ZYRON-DASHBOARD-v1'
...
def _ensure_network_access(port: int) -> None:
    ...
    if sys.platform == "win32":
        ...
        port_rule = f"ZYRON Dashboard Port {port}"
        prog_rule  = "ZYRON Dashboard Python"
        ...
        bat_body = "\r\n".join(bat_lines) + "\r\n"
        fd, bat_path = tempfile.mkstemp(suffix=".bat", prefix="zyron_fw_")
        try:
        ...
        try:
            ...
            if int(ret) > 32:
                ...
                print("[Dashboard] Phone connections may fail until ZYRON is run as Administrator.")
...
class DashboardServer:
    ...
    def _ssl_enabled() -> bool:
        ...
        return (certs / "zyron.key").exists() and (certs / "zyron.crt").exists()
...
<p>Press <strong style="color:#dde3ed">Remote Control</strong> in ZYRON to get a new QR code.</p>
</div></body></html>""")
...
<body>
<script>
  sessionStorage.setItem('zyron_token','{tok}');
  sessionStorage.setItem('zyron_key','{key}');
  localStorage.setItem('zyron_device_token','{dev_tok}');
  setTimeout(function(){{location.replace('/')}},400);
</script>
<p>Connecting to ZYRON…</p>
</body></html>""")
    ...
    async def _serve_alias(self) -> None:
        ...
        ssl_key  = BASE_DIR / "config" / "certs" / "zyron.key"
        ssl_cert = BASE_DIR / "config" / "certs" / "zyron.crt"
    ...
    async def serve(self) -> None:
        ...
        print("[Dashboard] Press 'Remote Control' in ZYRON UI to get the QR code.")
        await uvicorn.Server(cfg).serve()
